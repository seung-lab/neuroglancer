...
def tfclass(cls):
    ...
    """
    # return cls
    for name, method in inspect.getmembers(cls, lambda x: inspect.ismethod(x)):
        setattr(cls, name, tfscope(method, classname=cls.__name__))
    return cls

def tfscope(fn, classname=None):
    source = dedent(inspect.getsource(fn))
    _ast = ast.parse(source)
    Transformer().visit(_ast)
    ast.fix_missing_locations(_ast)
    compiled = compile(_ast, filename="<string>", mode="exec")
...
class Transformer(ast.NodeTransformer):
    ...
    def visit_Assign(self, node):
        """
            Assign(
                targets=[Name(id='var_a', ctx=Store())],
                value=Call(func=Attribute(value=Name(id='tf', ctx=Load()),
                           attr='Variable', ctx=Load()),
                           args=[Name(id='a', ctx=Load())], keywords=[], starargs=None, kwargs=None))


            Attribute(value=Attribute(value=Name(id='tf', ctx=Load()), attr='nn', ctx=Load()),
                     attr='sigmoid_cross_entropy_with_logits', ctx=Load())

        """
        try:
            if len(node.targets) == 1 and type(node.targets[0]) is ast.Name:
                ...
                if self.accepts_name(func_name):
                    for kw in node.value.keywords:
                        ...
                        node.value.keywords.append(
                            ast.keyword(arg='name', value=ast.Str(s=var_name)))
        except:
            pass
    ...
    def accepts_name(self, name):
        ...
        if 'name' in inspect.getargspec(func).args:
            return True
        else:
            return False
...
class Example(object):
    ...
    def some_function(self, a):
        var_a = tf.Variable(a, name='this_var_has_a_name')
        another_var = tf.Variable(5)
        return var_a + another_var
    ...
    def func(self):
        ...
        result = tf.nn.sigmoid_cross_entropy_with_logits(
            logits=0,
            labels=1)
        result = tf.contrib.layers.bias_add(0)
        return result

if __name__ == '__main__':
    Example().some_function(1)
