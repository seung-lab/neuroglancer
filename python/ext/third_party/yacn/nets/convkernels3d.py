...
from neuroglancer.pipeline.error_detection_correction.graph_naming import tfscope, tfclass
...
def bias_variable(schema):
	if type(schema) is FeatureSchema:
		return make_variable([1,1,1,1,schema.nfeatures])
	else:
		raise Exception("Unkown schema type")
...
class ConvKernel2d(ConvKernel):
	def __init__(self, size=(4,4), strides=(2,2), n_lower=1, n_upper=1,stddev=0.5):
		...
		self.weights=tf.Variable(initial, dtype=dtype, name='weights')
		self.size=size
...
class ConvKernelZ(ConvKernel):
	def __init__(self, size=2, n_lower=1, n_upper=1, stddev=0.5):
		...
		self.weights = tf.Variable(initial, dtype=dtype, name='weights', trainable=True)
		self.size = size
...
class ConvKernel3d(ConvKernel):
	def __init__(self, size=(1,4,4), strides=(1,2,2),
	             n_lower=1, n_upper=1,
		...
		# up_coeff and down_coeff are coefficients meant to keep
		# the magnitude of the output independent of stride and size choices
	...
	def transpose_call(self, x):
		...
		return tf.nn.conv3d_transpose(x, 
			self.down_coeff * self.weights,
			output_shape= full_in_shape,
...
class MultiscaleConv3d():
	...
	def __call__(self, inpt):
		ret = [0 for i in xrange(self.n)]
		for i in xrange(self.n):
			with tf.name_scope('unit'+str(i)):
				z=zero(self.otpt_schemas[i])
				l = [
						(z if i==self.n-1 else self.diag_connections[i](inpt[i+1])),
