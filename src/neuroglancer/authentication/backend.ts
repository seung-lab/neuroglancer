...
const authTokenSharedValuePromise: Promise<SharedWatchableValue<string|null>> =
    new Promise((f) => {
...
async function reauthenticate(auth_url: string, authTokenSharedValue: SharedWatchableValue<string|null>): Promise<string> {
    if (waitingForToken) {
    ...
    // returns a promise though I don't use this value since it also gets sent through the authTokenSharedValue
    // maybe it should be changed to a regular invoke
    rpc.invoke(AUTHENTICATION_REAUTHENTICATE_RPC_ID, {auth_url: auth_url, used_token: authTokenSharedValue.value});
