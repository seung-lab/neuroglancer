...
/**
 * @file User interface for interacting with a graph-enabled segmentation layer.
 */
...
export class GraphOperationSegmentListWidget extends RefCounted {
  ...
  constructor(
      ...
      public annotationLayer: GraphOpLayerState) {
    super();
...
export class SelectedGraphOperationState extends RefCounted {
  ...
  private annotationLayer: GraphOpLayerState|undefined;
  ...
  constructor(public annotationLayerState: Owned<WatchableRefCounted<GraphOpLayerState>>) {
    super();
...
export class GraphOperationLayerView extends Tab {
  ...
  constructor(
      public wrapper: Borrowed<GraphOperationLayer>,
      public state: Owned<SelectedGraphOperationState>,
      public annotationLayer: Owned<GraphOpLayerState>,
    ...
    wrapper.initializeGraphOperationLayerViewTab(this);

    {
      ...
      pointButton.addEventListener('click', () => {
        this.wrapper.layer.tool.value = new PlaceGraphOperationPointTool(this.wrapper, {});
      });
      ...
      confirmButton.addEventListener('click', () => {
        ...
        for (const annotation of sourceA) {
          if (annotation.type === AnnotationType.POINT &&
            annotation.segments && annotation.segments.length === 2) {
              const spatialPoint = vec3.transformMat4(vec3.create(), annotation.point, objectToGlobal);
        ...
        this.wrapper.layer.chunkedGraphLayer!.splitSegments(sources, sinks).then((splitRoots) => {
...
export class GraphOperationDetailsTab extends Tab {
  ...
  private segmentListWidget: GraphOperationSegmentListWidget|undefined;
  constructor(
  ...
  private updateView() {
    ...
    if (segmentListWidget === undefined) {
      this.segmentListWidget = segmentListWidget =
          this.registerDisposer(new GraphOperationSegmentListWidget(reference, annotationLayer));
    }
...
export class GraphOperationTab extends Tab {
  private stack = this.registerDisposer(
      new StackView<GraphOpLayerState, GraphOperationLayerView>(annotationLayerState => {
  ...
  private detailsTab = this.registerDisposer(
      new GraphOperationDetailsTab(this.state, this.voxelSize.addRef(), this.setSpatialCoordinates));
  constructor(
      public layer: Borrowed<GraphOperationLayer>,
...
function getSelectedAssociatedSegment(annotationLayer: GraphOpLayerState) {
...
abstract class PlaceGraphOperationTool extends Tool {
  ...
  constructor(public layer: GraphOperationLayer, options: any) {
    super();
...
export class PlaceGraphOperationPointTool extends PlaceGraphOperationTool {
  constructor(layer: GraphOperationLayer, options: any) {
    super(layer, options);
  }
  ...
  trigger(mouseState: MouseSelectionState) {
    ...
    if (mouseState.active) {
      const annotation: Annotation = {
        ...
        point:
            vec3.transformMat4(vec3.create(), mouseState.position, graphOperationLayer.globalToObject),
