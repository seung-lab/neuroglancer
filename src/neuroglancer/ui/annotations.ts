...
// should layer be Borrowed?
export function makeAnnotationListElement(layer: UserLayerWithAnnotations, annotation: Annotation, state: AnnotationLayerState, gridTemplate: string, globalDimensionIndices: number[], localDimensionIndices: number[], showDescription = true): [HTMLDivElement, number[]] {
    ...
    if (annotation.description && showDescription) {
      ++numRows;
      const description = document.createElement('div');
    ...
    element.addEventListener('action:move-to-annotation', event => {
      console.log('move-to-annotation');
      event.stopPropagation();
      event.preventDefault();
      const {layerRank} = chunkTransform;
