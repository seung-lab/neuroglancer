...
class PerspectiveRenderHelper extends RenderHelper {
  ...
  private shaderGetter = this.getDependentShader(
    "annotation/ellipsoid/projection",
...
class SliceViewRenderHelper extends RenderHelper {
  private shaderGetter = this.getDependentShader(
    "annotation/ellipsoid/crossSection",
    (builder: ShaderBuilder) => {
  ...
  draw(
    ...
    this.enable(this.shaderGetter, context, (shader) => {
      ...
      if (DEBUG) {
        ...
        const RviewportToObject = mat3.fromMat4(
          mat3.create(),
          viewportToObject,
        );
        const Aviewport = mat3.multiply(
          mat3.create(),
          mat3.transpose(mat3.create(), RviewportToObject),
          Aobject,
        );
        mat3.multiply(Aviewport, Aviewport, RviewportToObject);
        const cViewport = vec3.transformMat4(
          vec3.create(),
          center,
          objectToViewport,
        );
