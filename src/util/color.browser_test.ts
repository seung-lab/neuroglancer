...
describe("color", () => {
  ...
  it("serializeColor works", () => {
    expect(
      serializeColor(vec3.fromValues(0x12 / 255, 0x34 / 255, 0x56 / 255)),
    ).toEqual("#123456");
    expect(
      serializeColor(vec4.fromValues(101 / 255, 102 / 255, 103 / 255, 0.45)),
    ).toEqual("rgba(101, 102, 103, 0.45)");
  });

  it("parseRGBColorSpecification works", () => {
    expect(parseRGBColorSpecification("white")).toEqual(
      vec3.fromValues(1, 1, 1),
    );
    expect(parseRGBColorSpecification("black")).toEqual(
      vec3.fromValues(0, 0, 0),
    );
    expect(parseRGBColorSpecification("red")).toEqual(vec3.fromValues(1, 0, 0));
    expect(parseRGBColorSpecification("lime")).toEqual(
      vec3.fromValues(0, 1, 0),
    );
    expect(parseRGBColorSpecification("blue")).toEqual(
