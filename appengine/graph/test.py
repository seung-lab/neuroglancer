...
class TestOctree(unittest.TestCase):
    ...
    def test_add_atomic_node(self): #added
        tree = Octree(size=(4,1,3), path='/tmp/graph/0')
    ...
    def test_circle_external_edge(self):
        ...
        ext_node = labels.to_label(0, 0, 0, 1, 1)

        tree.add_atomic_node(1)          
        tree.add_atomic_node(2)
        tree.add_atomic_node(ext_node) 

        tree.add_atomic_edge(1, ext_node)

        #checks
        assert tree.get_node(1).parent !=  tree.get_node(ext_node).parent
        parent_chunk = tree.get_chunk(labels.to_chunk_key(1,0,0,0))
        intra_1 = labels.from_label(tree.get_node(1).parent)[-1]
        intra_ext = labels.from_label(tree.get_node(ext_node).parent)[-1]
        print intra_1, intra_ext
        assert parent_chunk.get_edge(intra_1, intra_ext)
        ...
        parent_chunk = tree.get_chunk(labels.to_chunk_key(1,0,0,0))
        intra_1 = labels.from_label(tree.get_node(1).parent)[-1]
        # print parent_chunk.get_edge(intra_1, intra_ext)

        # tree.add_atomic_edge(2,labels.to_label(0, 0, 0, 1, 1))
