...
class Octree(object):
    ...
    def _add_edge(self, label_u, label_v, edge):
        ...
        # delete current parents, and create a new node that combines them
    ...
    def _merge_node_recursive(self, label_u, label_v):
        ...
        #Keep track of edges to re add them later
        edges_u = list(chunk.get_chunk_edges(intra_u))
        edges_v = list(chunk.get_chunk_edges(intra_v))
        ...
        for u, v, edge in edges_u + edges_v:
            if intra_u == u:
                chunk.add_edge(new_parent, intra_v, edge)
            else:
                chunk.add_edge(intra_u, new_parent, edge)
